...
ds_used = 'Clothes_filtered.xlsx'

if ds_used == 'Clothes_filtered.xlsx':
    ...
    df = pd.get_dummies(df, columns=['Brand', 'Size', 'Status', 'Type'], drop_first=True)
...
elif ds_used == 'Sweet_filtered.xlsx':
    
        # Chargement du dataset depuis un fichier Excel
        df = pd.read_excel('Sweet_filtered.xlsx')
        ...
        # Transformation des données catégoriques en variables indicatrices
        df = pd.get_dummies(df, columns=['Brand', 'Size', 'Status'], drop_first=True)
